package team; //ver.9.8

/*영화 예매프로그램 : 설계

프로그램을 만드는 목적 및 기능 : 

손님은 상영중인 영화를 확인하고,영화티켓을 바로 구매하거나 장바구니에 담아 두고 수량 변경과 구매가 가능하다. 
...
class Movie {
	...
	// 티켓의 수량을 변경하는 메소드
	public void changeTicketCnt(int inputNum) {
...
public class TeamNum4_1 {
	...
	public static int findIndex(ArrayList<Movie> datas) {
		int i;
		while (true) { // 변경할 영화번호가 올바른 번호인지 확인하는 반복문
			boolean flag = false;
			System.out.println("해당 영화 번호를 입력하세요 : ");
			int inputNum = inputNumChk(); // 입력한 번호의 값을 검사하는 메소드 사용
	...
	public static void main(String[] args) {
		...
		int action;	// 프로그램 매뉴 입력값을 받는 변수 선언
		...
		while (true) { // 프로그램시작(무한루프)
			...
			action = inputNumChk(); // 입력한 번호의 값을 검사하는 메소드 사용
			if (action == 0) { // 종료조건 : 0을 입력받으면 프로그램 종료
				...
				while (true) { // 관리자 모드 시작(무한루프)
					...
					if (action == 0) { // 종료조건 : 0을입력받으면 프로그램 종료
						...
						objectI = datas.get(findIndex(datas));  
						...
						printList(datas);	// 배열리스트에 저장된 영화 목록을 출력 메소드 사용
						i = findIndex(datas); 	// 변경할 영화의 입력값을 검사하는 메소드 사용
						objectI = datas.get(i);
				...
				while (true) {// 티켓 구매 반복문 (올바르게 입력되는지)
					...
					if (action == 1) { // PK로 객체 찾기
						...
						while (true) {// 올바른 티켓 갯수를 입력할 때까지 확인하는 반복문 , 음수를 입력하는 경우 안내문구 추가
							...
							inputNum = inputNumChk(); // 입력한 번호의 값을 검사하는 메소드 사용
							if (inputNum == 0) {
								printWrongNum();// 올바른 숫자를 입력하지 않을때 메세지 출력
								continue;
							}
							...
							objectI.changeTicketCnt(inputNum);// 티켓 잔량을 감소 시키는 메서드 호출
				...
				// 장바구니 관련 코드
				/*
				 * 장바구니 기능만 우선적으로 구현.장바구니에 티켓이 들어오면 지금 담은 티켓 배열 출력해서 보여주기
				 * 
				 ...
				 * 영화 종류가 많아도 한 번에 다 살 수 있도록 담은 티켓 수량과 가격을 계산하여 총 금액을 출력해줌
				 * 
				...
				while (true) { 
					...
					if (action == 0) { // 종료조건
						...
						while (true) {
							for (Movie data : cart) { //장바구니에 담겨있는 티켓수량과 영화정보, 금액 확인
								System.out.println("[" + data.getNum() + "] [" + data.getName() + "] " + "가격 :"
										+ data.getPrice() + "원 수량 :" + data.getCartCnt() + "매");
							}
							System.out.println("총 금액은 " + total + "원입니다.");
							...
							System.out.println("맞으시면 1, 장바구니로 돌아가시려면 2를 입력해주세요 ");
							action = inputNumChk();
							if (action == 1) { // 이중for문 이용하여 , 카트인덱스의 번호와 무비인덱스의 번호가 같은지 확인

								for (i = 0; i < cart.size(); i++) {
									Movie cartIndex = cart.get(i);
									for (int j = 0; j < datas.size(); j++) {
										Movie movieIndex = datas.get(j);
										if (cartIndex.getNum() == movieIndex.getNum()) { 
											// 티켓의 수량을 변경하는 메소드 사용
											movieIndex.changeTicketCnt(cartIndex.getCartCnt());
										}
									}
								}
								cart.clear(); //구매 완료 후 장바구니 비워주기
					...
					} else if (action == 2) { // 장바구니에 담아둔 티켓의 갯수를 변경
						
						i = findIndex(cart);	// 변경할 영화의 입력값을 검사하는 메소드 사용
						objectI = cart.get(i);
						...
						while (true) {
							...
							inputNum = inputNumChk();	// 입력한 번호의 값을 검사하는 메소드 사용
							// 변경할 영화의 입력값을 검사하는 메소드 사용
							if (0 <= inputNum && inputNum <= datas.get(findIndex(datas)).getCnt()) {
