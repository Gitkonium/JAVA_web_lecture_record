...
class Bakery extends Restaurant {
	...
	Bakery(String shopName, String cignatureMenu, int price) {
		// 부모클래스의 생성자를 호출해서 멤버 변수를 초기화 한다
	...
	void event() {
		...
		if (star <= 3) {
			...
			// 가격 10% 할인
			double price=this.price * 0.9;

			System.out.println("10% 할인되어 가격은 " + price + "입니다.");
...
class Cafe extends Restaurant {
	...
	// 부모클래스의 메서드를 자식 클래스에 출력값과 함께 사용할 수 있게 재정의 한다
	@Override
	void provideFood() {

		System.out.println(this.shopName + "의 시그니처 메뉴는 " + this.cignatureMenu + "입니다.");
